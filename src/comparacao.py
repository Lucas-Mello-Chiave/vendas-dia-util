...
def gerar_historico_dias_uteis(arquivo_reduzido: str, historico_saida: str) -> None:
    ...
    # 3) Determina o intervalo completo (primeiro ao último mês do histórico)
    ...
    # 4) Gera todas as datas úteis (segunda a sexta) no intervalo
    all_dates = pd.date_range(start=start, end=end, freq='B')
    
    # 5) Remove feriados brasileiros
    br_holidays = holidays.Brazil(years=range(start.year, end.year + 1))
    all_dates = [d for d in all_dates if d not in br_holidays]
    ...
    print(f"[OK] Histórico com dias úteis (excluindo feriados brasileiros) salvo em: {historico_saida}")